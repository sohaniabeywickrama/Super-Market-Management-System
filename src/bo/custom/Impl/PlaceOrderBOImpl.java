...
public class PlaceOrderBOImpl implements PlaceOrderBO {
    ...
    public ObservableList<ItemDTO> getAllItem() throws ClassNotFoundException, SQLException {
        ...
        for (Item ID : all) {
            ItemDTO dto = new ItemDTO(ID.getItemCode(), ID.getSupplierID(), ID.getDescription(), ID.getPackSize(), ID.getUnitPrice(), ID.getQtyOnHand(), ID.getBatchID());
            allItem.add(dto);
        }
        return allItem;
    }
    ...
    public ItemDTO searchItem(String itemCode) throws Exception {
        ...
        return new ItemDTO(search.getItemCode(), search.getSupplierID(), search.getDescription(), search.getPackSize(), search.getUnitPrice(), search.getQtyOnHand(), search.getBatchID());
    }
    ...
    public boolean placeOrder(OrdersDTO dto) throws ClassNotFoundException, SQLException, Exception {
        ...
        Orders orders = new Orders(dto.getOrderID(), dto.getOrderDate(), dto.getOrderTime(), dto.getCustName(), dto.getCustPhoneNo(), dto.getCustAddress(), dto.getCustEmail(), dto.getCustID());
    ...
    private boolean addOrderDetail(OrdersDTO dto) throws SQLException, ClassNotFoundException {
        for (dtmTM ord : dto.getAllOrderDetail()) {
            Orderdetail orderTable = new Orderdetail(dto.getOrderID(), ord.getCode().substring(0,4), ord.getQTY(), ord.getPrice());
            System.out.println("ord.getCode()");
            System.out.println(ord.getCode().substring(0,4));
    ...
    public ObservableList<CustomDTO> searchOrderFromID(String id) throws ClassNotFoundException, SQLException, Exception {
        ...
        for (CustomEntity C : od) {
            CustomDTO dto = new CustomDTO(C.getOrderID(), C.getItemCode(), C.getOrderQTY(), C.getUnitPrice(), C.getDiscount(), C.getOrderDate(), C.getOrderTime(), C.getCustName(), C.getCustPhoneNo(), C.getCustAddress(), C.getCustEmail(), C.getCustID());
            allDetails.add(dto);
        }
        return allDetails;
    }
